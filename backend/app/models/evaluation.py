...
class EvalRun(Base):
    ...
    dimension = Column(SQLEnum(DatasetDimension, name="eval_dataset_dimension_enum", values_callable=lambda x: [e.value for e in x]), nullable=False, default=DatasetDimension.TWO_D.value)

    # Foreign keys
    model_id = Column(UUID(as_uuid=True), ForeignKey("od_models.id", ondelete="RESTRICT"), nullable=False)
    # 2D / 3D datasets (front-end will pick based on dimension)
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            # Pre-attack: need one base dataset (2D or 3D), no attack dataset
            "(phase = 'pre_attack' AND (base_dataset_id IS NOT NULL OR base_dataset_3d_id IS NOT NULL) "
            "AND attack_dataset_id IS NULL AND attack_dataset_3d_id IS NULL) "
            "OR "
            # Post-attack: base present, exactly one attack dataset (2D or 3D)
            "(phase = 'post_attack' AND (base_dataset_id IS NOT NULL OR base_dataset_3d_id IS NOT NULL) "
            "AND ((attack_dataset_id IS NOT NULL AND attack_dataset_3d_id IS NULL) "
            "OR (attack_dataset_id IS NULL AND attack_dataset_3d_id IS NOT NULL)))",
...
class EvalItem(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="chk_eval_item_image_xor"
        ),
    )

    def __repr__(self):
        return f"<EvalItem(id={self.id}, run_id={self.run_id}, image_2d_id={self.image_2d_id}, image_3d_id={self.image_3d_id})>"
