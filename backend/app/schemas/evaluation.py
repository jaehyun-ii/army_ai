...
class EvalRunBase(BaseModel):
    ...
    phase: EvalPhase
    model_id: UUID
    # Dimension hint for front-end branching
    dimension: DatasetDimension = Field(DatasetDimension.TWO_D, description="Dataset dimension: 2d or 3d")
    # 2D / 3D datasets (front-end will set one pair)
...
class EvalItemResponse(EvalItemBase):
    ...
    PHASE 2 UPDATE: file_name and storage_key are computed from image relationships.
    """
    id: UUID
    created_at: datetime
    updated_at: datetime
    ...
    # Computed fields from image relationships (for backward compatibility)
    file_name: Optional[str] = Field(None, description="Computed from image_2d/file_name or image_3d.file_name")
    storage_key: Optional[str] = Field(None, description="Computed from image_2d/storage_key or image_3d.storage_key")
