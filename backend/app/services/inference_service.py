...
class InferenceService:
    ...
    async def run_inference(
        ...
        for image_id in image_ids:
            # Fetch image metadata based on dimension
            if dimension == "3d":
                from sqlalchemy import select
                from app.models.dataset_3d import Image3D
                result = await db.execute(select(Image3D).where(Image3D.id == image_id, Image3D.deleted_at.is_(None)))
    ...
    async def batch_inference(
        ...
        if dimension == "3d":
            ...
            result = await db.execute(
                select(Image3D).where(Image3D.dataset_id == dataset_id, Image3D.deleted_at.is_(None))
            )
