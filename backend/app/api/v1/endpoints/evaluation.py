...
async def execute_evaluation_run(
    ...
    target_class: Optional[str] = Query(None, description="Target class to evaluate (filters predictions and GT)"),
    ...
    If target_class is provided, only predictions and GT for that class will be evaluated.
    ...
    # Save evaluation parameters including target_class
    params = db_eval_run.params or {}
    ...
    final_target_class = target_class
    if not final_target_class and params.get('target_class'):
        final_target_class = params['target_class']
        ...
        async with AsyncSessionLocal() as bg_db:
            try:
                await evaluation_service.execute_evaluation(
                    ...
                    target_class=final_target_class,  # Use final_target_class from params
...
async def visualize_evaluation_item(
    ...
    # Get target class from eval run params
    eval_run = await crud_evaluation.get_eval_run(db=db, eval_run_id=eval_item.run_id)
    target_class = None
    if eval_run and eval_run.params:
        target_class = eval_run.params.get('target_class')
    # Also check metrics_summary for target_class (after evaluation)
    if not target_class and eval_run and eval_run.metrics_summary:
        target_class = eval_run.metrics_summary.get('target_class')

    # Log for debugging
    import logging
    logger = logging.getLogger(__name__)
    logger.info(f"[VISUALIZE] Item {item_id}, Run {eval_item.run_id}, Target class: {target_class}")
    ...
    logger.info(f"[VISUALIZE] Original predictions count: {original_count}")
    if predictions and len(predictions) <= 5:
        logger.info(f"[VISUALIZE] All predictions: {[p.get('class_name') for p in predictions]}")

    # Filter predictions by target class if specified
    if target_class:
        predictions = [p for p in predictions if p.get('class_name') == target_class]
        logger.info(f"[VISUALIZE] Filtered predictions: {original_count} -> {len(predictions)} (target: {target_class})")

    if predictions and len(predictions) <= 5:
        logger.info(f"[VISUALIZE] Final predictions: {[p.get('class_name') for p in predictions]}")
