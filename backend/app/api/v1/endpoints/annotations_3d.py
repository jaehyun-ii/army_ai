"""
Annotation endpoints for 3D datasets - retrieving and managing detection/inference results.
"""
...
async def get_image_3d_annotations(
    image_id: UUID,
    ...
    """Get all annotations for a specific 3D image.

    Args:
        image_id: UUID of the 3D image
    ...
    try:
        ...
        query = select(models.Annotation).where(
            models.Annotation.image_3d_id == image_id,
        ...
        logger.error(f"Error retrieving annotations for 3D image {image_id}: {str(e)}", exc_info=True)
        raise InternalServerError(detail=f"Error retrieving annotations: {str(e)}")


@router.get("/dataset/{dataset_id}", response_model=dict)
async def get_dataset_3d_annotations_summary(
    dataset_id: UUID,
    db: AsyncSession = Depends(get_db),
    min_confidence: Optional[float] = Query(None, ge=0.0, le=1.0, description="Minimum confidence threshold"),
) -> dict:
    """Get annotation summary for all images in a 3D dataset.
    ...
    Args:
        dataset_id: UUID of the 3D dataset
        min_confidence: Optional minimum confidence threshold
    ...
    try:
        ...
        dataset_query = select(models.Dataset3D).where(
            models.Dataset3D.id == dataset_id,
            models.Dataset3D.deleted_at.is_(None)
        )
        dataset_result = await db.execute(dataset_query)
        dataset = dataset_result.scalar_one_or_none()

        if not dataset:
            raise NotFoundError(resource=f"3D Dataset {dataset_id}")

        # Get all images in dataset
        images_query = select(models.Image3D).where(
            models.Image3D.dataset_id == dataset_id,
            models.Image3D.deleted_at.is_(None)
        )
        ...
        annotations_query = select(models.Annotation).where(
            models.Annotation.image_3d_id.in_(image_ids),
            models.Annotation.deleted_at.is_(None)
        )
        ...
        raise InternalServerError(detail=f"Error retrieving 3D dataset annotations: {str(e)}")
...
async def create_annotations_3d_bulk(
    ...
    image_id: UUID = Query(..., description="3D Image ID for all annotations"),
    db: AsyncSession = Depends(get_db),
) -> List[schemas.AnnotationResponse]:
    """Create multiple annotations for a 3D image at once.

    Useful for batch uploading detection results.

    Args:
        annotations: List of annotation data to create
        image_id: UUID of the 3D image these annotations belong to
    ...
    try:
        # Verify image exists
        image_query = select(models.Image3D).where(
            models.Image3D.id == image_id,
            models.Image3D.deleted_at.is_(None)
        )
        ...
        if not image:
            raise NotFoundError(resource=f"3D Image {image_id}")

        # Create annotation records
        created_annotations = []
        for ann_data in annotations:
            ann_dict = ann_data.model_dump(exclude_unset=True)
            ann_dict["image_3d_id"] = image_id
...
async def delete_image_3d_annotations(
    ...
    try:
        ...
        if not annotations:
            raise NotFoundError(resource=f"No annotations for 3D image {image_id}")
