...
class GlobalCameraState:
    ...
    def __init__(self):
        ...
        self.capture_current: int = 0
        self.capture_total: int = 0
...
async def stream_camera(
    draw_boxes: bool = Query(True, description="Whether to include detection results in the stream")
):
    """
    SSE stream that delivers base64-encoded frames and detection metadata.
    """
    logger.info(f"SSE stream requested with draw_boxes={draw_boxes}, estimator_id={global_camera.estimator_id}")
    ...
    if not cap:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="No active capture found for stream"
        )
    ...
    async def generate_events():
        ...
        # Target FPS for streaming (lower = more stable)
        target_fps = 20
        ...
        while global_camera.is_active:
            try:
                ...
                # Send heartbeat comment every 5 seconds to keep connection alive
                if current_time - last_heartbeat_time >= 5.0:
                    yield f": heartbeat\n\n"
                    await asyncio.sleep(0)
                    last_heartbeat_time = current_time

                # Clear buffer more aggressively to get latest frame
                try:
                    # Discard old buffered frames
                    for _ in range(3):
                        if not cap.grab():
                            break

                    ret, frame = await loop.run_in_executor(None, cap.retrieve)
                except Exception as read_error:
                    logger.warning(f"Frame read error: {read_error}")
                ...
                if global_camera.estimator_id and draw_boxes:
                    try:
                        inference_start = time.time()
                        # Add timeout for inference
                        inference_task = model_inference_service.run_inference(
                            version_id=global_camera.estimator_id,
                            image=frame,
                            conf_threshold=0.25,
                            iou_threshold=0.45
                        )
                        result = await asyncio.wait_for(inference_task, timeout=1.0)
                        ...
                        logger.warning(f"Inference timeout on frame {frame_count}")
                    except Exception as e:
                        logger.warning(f"Detection error on frame {frame_count}: {e}")

                # Lower JPEG quality for faster encoding/transmission (60 is good balance)
                ...
                # Include stats in every frame to avoid separate events
                frame_payload = {
                    "frame": frame_base64,
                ...
                logger.error(f"Error generating SSE frame: {e}", exc_info=True)
                # Don't break on errors, try to recover
...
async def start_capture_session(
    ...
    try:
        ...
        if request.name:
            ...
            await db.commit()
            logger.info(f"Updated capture run {global_camera.run_id} with name: {request.name}")

        logger.info(f"Starting frame capture for run {global_camera.run_id}")

        # Capture configuration
        target_frame_count = 10  # Always capture exactly 10 frames
        frame_interval = 0.5  # 0.5 seconds between frames (total 5 seconds for 10 frames)

        # Set capture state for SSE updates
        global_camera.is_capturing = True
        global_camera.capture_current = 0
        global_camera.capture_total = target_frame_count
        ...
        # Capture exactly target_frame_count frames
        frames_captured = 0
        max_attempts = target_frame_count * 3  # Allow up to 3x attempts to handle failures
        attempt_count = 0

        while frames_captured < target_frame_count and attempt_count < max_attempts:
            attempt_count += 1
            ...
            if ret and frame is not None:
                ...
                if ret_encode:
                    try:
                        ...
                        for det in detections_list:
                            try:
                                # Stored detections are normalized xyxy; convert to normalized center-width-height
                                x1 = det["bbox"]["x1"]
                                y1 = det["bbox"]["y1"]
                                x2 = det["bbox"]["x2"]
                                y2 = det["bbox"]["y2"]
                                ...
                                bbox_x = x1 + bbox_width / 2
                                bbox_y = y1 + bbox_height / 2
                        ...
                        seq_no += 1
                        frames_captured += 1  # Increment successful capture count
                        # Update capture progress for SSE
                        global_camera.capture_current = frames_captured
                    except Exception as e:
                ...
                # Wait before capturing next frame (only if not the last frame)
                if frames_captured < target_frame_count:
                    await asyncio.sleep(frame_interval)
            else:
                # Frame read failed, wait briefly before retrying
                logger.warning(f"Failed to read frame (attempt {attempt_count}/{max_attempts})")
                await asyncio.sleep(0.1)  # Short delay before retry

        # Log capture completion status
        if frames_captured < target_frame_count:
            logger.warning(
                f"Capture ended early: captured {frames_captured}/{target_frame_count} frames "
                f"after {attempt_count} attempts"
            )
        else:
            logger.info(f"Successfully captured all {frames_captured} frames for run {global_camera.run_id}")
        ...
        # Calculate actual duration based on frames captured
        estimated_duration = frames_captured * frame_interval

        # Reset capture state but keep final counts so SSE can show completion
        global_camera.is_capturing = False
        global_camera.capture_current = frames_captured
        global_camera.capture_total = target_frame_count
...
async def list_captures(
    ...
    try:
        ...
        for run, frame_count in rows:
            ...
            captures.append({
                "id": str(run.id),
                "name": run.name,  # Include capture name
