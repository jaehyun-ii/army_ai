...
class MaintenanceModeMiddleware(BaseHTTPMiddleware):
    """
    Middleware to block all API requests during maintenance mode (e.g., during restore).
    """
    def __init__(self, app: ASGIApp):
        super().__init__(app)

    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        # Allow health checks and docs even during maintenance
        ...
        if is_maintenance_mode():
            return JSONResponse(
                ...
                content={
                    "detail": "System is currently under maintenance (restore in progress). Please try again later.",
...
class SystemLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        logger.info(f"🔵 REQUEST [{request_id[:8]}] {request.method} {request.url.path}")
        ...
        logger.info(f"{status_emoji} RESPONSE [{request_id[:8]}] {response.status_code} {request.method} {request.url.path} ({process_time:.2f}ms)")
        ...
        # Log to database (skip during maintenance mode to avoid connection errors)
        # IMPORTANT: Do NOT await logging - run in background to avoid blocking response
        if not is_maintenance_mode():
            import asyncio

            async def log_in_background():
                try:
                    ...
                    logger.error(f"Failed to log request to database: {type(e).__name__}: {str(e)}", exc_info=False)

            # Run logging in background - do NOT await
            asyncio.create_task(log_in_background())
