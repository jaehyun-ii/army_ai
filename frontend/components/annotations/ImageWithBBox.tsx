...
interface ImageWithBBoxProps {
  imageId: string
  imageData?: string // Base64 encoded image data (deprecated, use storageKey)
  storageKey?: string // Storage key for loading image via /api/storage (preferred)
...
export function ImageWithBBox({
  ...
  // Generate image URL from storageKey or imageData
  const imageUrl = storageKey
    ? `/api/storage/${storageKey.startsWith('storage/') ? storageKey.substring('storage/'.length) : storageKey}`
    : imageData
