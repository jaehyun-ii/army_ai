...
export function ImageSelector({ datasetId, className = '' }: ImageSelectorProps) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-muted">이미지 로딩 중...</div>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="p-4">
        <div className="bg-red-100 border border-error text-red-700 px-4 py-3 rounded">
          {error}
        </div>
      </div>
    )
        ...
        {selectedImage && selectedImageId && (
          <div className="space-y-4">
            ...
            <AnnotatedImageViewer
              ...
              imageUrl={`/api/storage/${selectedImage.storage_path}`}
              minConfidence={0.3}
              showLabels={true}
            />
          </div>
