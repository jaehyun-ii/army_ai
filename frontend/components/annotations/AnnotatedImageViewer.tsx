...
export function AnnotatedImageViewer({
  ...
  return (
    <Card className={`relative ${className}`}>
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-scrim/50 z-10">
          <div className="text-foreground">어노테이션 로딩 중...</div>
        </div>
      )}

      {error && (
        <div className="absolute top-4 left-4 right-4 z-10">
          <div className="bg-red-500 text-foreground p-4 rounded">
            {error}
          </div>
        </div>
      )}
