    ...
    <Card className={cn(styles.card.base, className)}>
      ...
      <CardContent className="p-4">
        <div className="border border-border rounded-lg overflow-hidden">
          <Table>
            <TableHeader>
              <TableRow className="border-border bg-surface-container/50">
                {columns.map((column, index) => (
                  <TableHead
                    key={index}
                    className={cn(
                      "text-foreground font-semibold h-12 px-6",
                      column.width,
                ...
                {actions && <TableHead className="text-right text-foreground font-semibold h-12 px-6">작업</TableHead>}
              </TableRow>
            </TableHeader>
            <TableBody>
              {data.length === 0 ? (
                <TableRow>
                  <TableCell
                    ...
                    className="text-center text-muted py-12 px-6"
                  >
                    {emptyMessage}
                  </TableCell>
                </TableRow>
                ...
                data.map((item, rowIndex) => (
                  <TableRow
                    key={rowIndex}
                    className={cn(
                      "border-border hover:bg-surface-container-high/40 transition-colors",
                      onRowClick && "cursor-pointer",
                      selectedRow === item && "bg-surface-container-high/50",
                      rowIndex % 2 === 0 ? "bg-background/20" : "bg-background/40"
                    )}
...
export function StatusBadge({ status, variant }: { status: string; variant?: any }) {
  const getStatusStyle = (status: string) => {
    ...
    if (lowerStatus === "대기" || lowerStatus === "pending" || lowerStatus === "waiting") {
      return "bg-warning-container text-tertiary border-warning"
    }
    if (lowerStatus === "실패" || lowerStatus === "failed" || lowerStatus === "error") {
      return "bg-error-container text-error border-error"
    }
    return "bg-surface-container/50 text-muted border-outline/30"
  }
