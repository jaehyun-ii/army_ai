      ...
      const transformedDatasets: AdversarialDataset[] = await Promise.all(
        datasetsData.map(async (d: AttackDatasetAsset) => {
          ...
          return {
            ...
            // Use image_count from backend (which is calculated from actual Image2D records)
      ...
      <div>
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "patches" | "patch-datasets" | "noise-datasets")}>
          <TabsList className="flex flex-col md:grid md:grid-cols-3 gap-1 bg-surface-container-low p-1 h-auto w-full">
            ...
            <TabsTrigger value="patch-datasets" className="h-8 px-2 py-1 w-full justify-center">
              <Database className="w-3 h-3 mr-1 flex-shrink-0" />
              <span className="text-xs truncate">패치 데이터셋</span>
            </TabsTrigger>
            <TabsTrigger value="noise-datasets" className="h-8 px-2 py-1 w-full justify-center">
              <FileStack className="w-3 h-3 mr-1 flex-shrink-0" />
              <span className="text-xs truncate">노이즈 데이터셋</span>
            </TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
