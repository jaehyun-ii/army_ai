...
class APIClient {
  ...
  async executeEvaluationRun(
    ...
    if (params.target_class !== undefined && params.target_class !== '') {
      queryParams.append('target_class', params.target_class)
    }

    const url = `/api/evaluations/${runId}/execute?${queryParams.toString()}`
    console.log('[API Client] Executing evaluation:', url, 'params:', params)
