...
export interface TrainingConfig {
  ...
  intensity_level?: "weak" | "medium" | "strong" // Added: intensity level
  iterations?: number
  patch_size?: number
  learning_rate?: number | null // Added: learning rate parameter
  session_id: string  // Added: Pre-generated session ID for SSE connection
}

export interface TrainingResponse {
  training_id?: string | number  // Optional: may not be returned immediately
  session_id: string  // SSE session ID for progress tracking
  status: string  // "processing" when background task started
  message: string
  sse_url?: string
  patch?: any  // Optional: legacy format support
  patch_file_path?: string  // Optional: legacy format support
}
...
export interface AdversarialDataResponse {
  generation_id?: string  // Optional: may not be returned immediately
  output_dataset_id?: string | null
  session_id?: string  // SSE session ID for progress tracking
  status?: string  // "processing" when background task started
  message: string
  sse_url?: string
  attack_dataset?: any  // Optional: legacy format support
}
...
export async function startTraining(config: TrainingConfig): Promise<TrainingResponse> {
  try {
    ...
    const requestBody = {
      ...
      intensity_level: config.intensity_level, // Added: intensity level
    ...
    return {
      training_id: data.patch?.id || data.training_id || sessionId,
      session_id: data.session_id || sessionId,  // Use session_id from response or provided one
      status: data.status || 'processing',
      message: data.message || 'Patch generation started',
      sse_url: data.sse_url || `/api/patches/sse/${sessionId}`
    }
  } catch (error) {
...
export async function startAdversarialDataGeneration(config: AdversarialDataConfig): Promise<AdversarialDataResponse> {
  try {
    ...
    return {
      generation_id: data.attack_dataset?.id || data.id || config.session_id || '',
      output_dataset_id: outputDatasetId,
      session_id: data.session_id || config.session_id,
      status: data.status || 'processing',
      message: data.message || 'Attack dataset generation started',
      sse_url: data.sse_url,
      attack_dataset: data.attack_dataset
    }
  } catch (error) {
...
export async function previewPatchOnImage(request: PatchPreviewRequest): Promise<PatchPreviewResponse> {
  try {
    // Fixed: Use correct backend endpoint
    const endpoint = `${BACKEND_API_URL}${API_V1}/patches/${request.patch_id}/preview`
    const response = await fetch(endpoint, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export function getPatchImageUrl(patchId: string): string {
  // Fixed: Use correct backend endpoint
  return `${BACKEND_API_URL}${API_V1}/patches/${patchId}/image`
}
