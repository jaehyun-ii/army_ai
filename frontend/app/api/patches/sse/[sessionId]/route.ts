...
export async function GET(
  ...
  try {
    ...
    console.log('[/api/patches/sse] SSE stream request for session:', sessionId)

    const backendResponse = await fetch(
      `${BACKEND_API_URL}/api/v1/patches/sse/${sessionId}`,
      {
        method: 'GET',
      }
    )
    ...
    if (!backendResponse.ok) {
      console.error('[/api/patches/sse] Backend error:', backendResponse.status)
    ...
    console.error('[/api/patches/sse] Error:', error)
