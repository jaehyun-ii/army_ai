...
export async function GET() {
  try {
    ...
    const url = `${backendUrl}/api/v1/carla/sim_weather_list`
    ...
    console.error('[CARLA API] Error fetching weather list:', error)
    return NextResponse.json(
      { state: 500, message: 'Failed to fetch weather list', result: [] },
      { status: 500 }
    )
  }
}
