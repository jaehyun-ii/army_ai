...
export async function GET() {
  try {
    ...
    const url = `${backendUrl}/api/v1/carla/sim_time_list`
    ...
    console.error('[CARLA API] Error fetching time list:', error)
    return NextResponse.json(
      { state: 500, message: 'Failed to fetch time list', result: [] },
      { status: 500 }
    )
  }
}
